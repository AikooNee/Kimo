...
module.exports = async (client, oldChannel, newChannel) => {
  ...
  const embed = new EmbedBuilder().setAuthor({ name: "Channel Updated" }).setTimestamp().setColor("Green");
  ...
  embed
    .setFooter({ text: `ID: ${newChannel.id} | Executor: ${executor ? executor.username : "Unknown"}` })
...
const getDifferrence = (oldOverwrites, newOverwrites) => {
  ...
  newOverwrites.forEach((newOverwrite, id) => {
    ...
    if (!oldOverwrite) {
      ...
      changes.push(
        `+ Added new overwrite for ${newOverwrite.type === OverwriteType.Member ? `<@${id}>` : `<@&${id}>`}: ${[...addedPermissions, ...deniedPermissions].join(", ")}`
      );
    } else {
      ...
      if (addedAllow.length || removedAllow.length || addedDeny.length || removedDeny.length) {
        changes.push(
          `+ Modified overwrite for ${newOverwrite.type === OverwriteType.Member ? `<@${id}>` : `<@&${id}>`}:`
        );
  ...
  oldOverwrites.forEach((oldOverwrite, id) => {
    if (!newOverwrites.has(id)) {
      ...
      changes.push(
        `- Removed overwrites for ${oldOverwrite.type === OverwriteType.Member ? `<@${id}>` : `<@&${id}>`}: ${[...removedPermissions, ...deniedPermissions].join(", ")}`
      );
    }
  });
