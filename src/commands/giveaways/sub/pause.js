...
module.exports = async (member, messageId) => {
  ...
  if (giveaway.pauseOptions.isPaused)
    return new EmbedBuilder()
      .setColor(EMBED_COLORS.BOT_EMBED)
      .setDescription("Senpai, this giveaway is already paused.");

  try {
    await giveaway.pause();
    return new EmbedBuilder()
      .setColor(EMBED_COLORS.BOT_EMBED)
      .setDescription("Success, senpai! The giveaway has been paused! ⏸️");
  } catch (error) {
    member.client.logger.error("Giveaway Pause", error);
    return new EmbedBuilder()
      .setColor(EMBED_COLORS.BOT_EMBED)
      .setDescription(`Senpai, an error occurred while pausing the giveaway: ${error.message} 💔`);
  }
};
