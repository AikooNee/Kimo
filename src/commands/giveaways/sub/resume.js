...
module.exports = async (member, messageId) => {
  ...
  if (!giveaway.pauseOptions.isPaused) return new EmbedBuilder().setColor(EMBED_COLORS.BOT_EMBED).setDescription("Senpai, this giveaway is not paused.");

  try {
    await giveaway.unpause();
    return new EmbedBuilder().setColor(EMBED_COLORS.BOT_EMBED).setDescription("Success, senpai! The giveaway has been revive!");
  } catch (error) {
    member.client.logger.error("Giveaway Resume", error);
    return new EmbedBuilder().setColor(EMBED_COLORS.BOT_EMBED).setDescription(`Senpai, an error occurred while unpausing the giveaway: ${error.message} 💔`);
  }
};
