...
async function getSnipe(context, data, user) {
    ...
    collector.on("collect", async (response) => {
      if (response.customId === "accept") {
        ...
        const embed = new EmbedBuilder()
          ...
          .setDescription("You have accepted the snipe policy. You can now use the snipe command to view last deleted messages.");
        await m.edit({ embeds: [embed], components: [] });
      } else if (response.customId === "reject") {
        const embed = new EmbedBuilder()
          .setColor(EMBED_COLORS.ERROR)
          .setDescription("You have rejected the snipe policy. You cannot use the snipe command to view last deleted messages");
        await m.edit({ embeds: [embed], components: [] });
      }
    });
  ...
  const content = settings.snipe.content.length > 1000 
    ? `${settings.snipe.content.substring(0, 1000)}...` 
    : settings.snipe.content;
  const embed = new EmbedBuilder()
    ...
    .addFields(
      ...
      { name: "Channel", value: `<#${settings.snipe.channel}>` }
    )
